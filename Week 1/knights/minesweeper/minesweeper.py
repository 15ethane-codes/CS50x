...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        while True: #4.
            ...
            if not safes.difference(self.safes) and not mines.difference(self.mines):
                break
            for safe in safes:
                self.mark_safe(safe)
            for mine in mines:
    ...
    def make_random_move(self):
        ...
        """
        all_moves = [(i,j) for i in range(self.height) for j in range(self.width) if (i,j) not in self.moves_made and (i,j) not in self.mines]
        if all_moves:
